...
app.post('/api/valet-bookings', async (req, res) => {
  ...
  try {
    ...
    // If valet_name not provided, fetch from database!
    let finalValetName = valet_name;
    if (!finalValetName) {
      const valetResult = await connection.query(
        'SELECT name FROM valets WHERE valet_id = $1',
        [valet_id]
      );
      if (valetResult.rows.length > 0) {
        finalValetName = valetResult.rows[0].name;
      }
    }
    ...
    await connection.query(
      `INSERT INTO valet_bookings (
        ...
        special_requirements, notes, source, prep_tracker_id, created_at
      ...
      [
        ...
        valet_id, finalValetName || null, 'pending', paint_protection || 'no',
        special_requirements || null, notes || null, source || 'manual', prep_tracker_id || null
      ]
    );
